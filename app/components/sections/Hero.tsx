...
export function Hero({ t, greeting, reduced, setMag, phase }: HeroProps) {
    ...
    const onMove = (e: MouseEvent) => {
      ...
      neighbors.forEach((neighbor) => {
        ...
        const distToNeighbor = Math.sqrt(
          Math.pow(e.clientX - nX, 2) + Math.pow(e.clientY - nY, 2),
        );
  ...
  return (
    ...
    >
      ...
      <div
        ref={memojiRef}
        className="memoji hidden lg:flex lg:flex-col items-center justify-start absolute right-0 top-0 w-1/2 h-full pb-20 z-0 pointer-events-none"
        aria-hidden="true"
      >
      ...
      <div className="h-txt flex-1 flex flex-col justify-center max-w-[1200px] mx-auto w-full px-8 py-8 relative z-[1]">
        <div className="max-w-full lg:max-w-[54%]">
          <div
            className="mb-5 overflow-hidden pb-2"
            ref={textContainerRef}
            style={{ isolation: "isolate" }}
          >
            <p className="h-ln text-[clamp(1.1rem,2.2vw,1.65rem)] font-medium text-lead mb-1">
              {greeting}
            </p>
          ...
          <p className="h-fd text-[clamp(1rem,2vw,1.4rem)] font-bold text-lead mb-2">
            {t.role}
          </p>
          <p className="h-fd text-[14px] text-lead max-w-[380px] mb-8">
            {t.tagline}
          </p>
      ...
      <div className="h-fd border-t border-black/7 dark:border-white/10 py-4 px-8 text-[10px] font-bold tracking-widest uppercase text-lead/30 flex items-center gap-2 max-w-[1200px] mx-auto w-full no-print">
        <div className="animate-bounce flex items-center gap-2">
          ...
          <svg width="18" height="5" viewBox="0 0 18 5" fill="none">
            <path
              d="M0 2.5h16M12.5 1l4 1.5-4 1.5"
              stroke="currentColor"
              strokeWidth=".8"
            />
          </svg>
