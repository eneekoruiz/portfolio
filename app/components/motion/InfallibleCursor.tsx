...
export function InfallibleCursor() {
  ...
  // Movement states
  const mx = useRef(-100); const my = useRef(-100);
  const cx = useRef(-100); const cy = useRef(-100);
  ...
  useEffect(() => {
    ...
    const onEnter = (e: MouseEvent) => {
      const target = (e.target as HTMLElement).closest('a,button,[data-h],input,select,[data-cursor-plus],[data-cursor-minus]') as HTMLElement;
      if (!target) return;
      ...
      else if (target.tagName === 'INPUT' || target.tagName === 'TEXTAREA') setMode('text');
      else if (target.tagName === 'A' && target.getAttribute('target') === '_blank') setMode('arrow');
  ...
  return (
    <div
      ...
      style={{
        ...
        opacity: 0, willChange: 'transform, scale',
        display: 'flex', alignItems: 'center', justifyContent: 'center',
        transition: 'opacity 0.3s ease, background-color 0.4s ease',
      }}
    >
      <div className="transition-all duration-300 flex items-center justify-center" style={{ transform: `scale(${isHover.current ? 0.3 : 0})`, color: 'black' }}>
