...
export function DebugHUD() {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key.toLowerCase() === 'd' && !['input', 'textarea'].includes((e.target as HTMLElement).tagName.toLowerCase())) {
        setActive(prev => !prev);
      }
    };
      ...
      <div className="relative group overflow-hidden rounded-xl border border-white/10 bg-black/40 backdrop-blur-md p-4 font-mono text-[10px] text-brand/80 shadow-2xl transition-all duration-500 hover:bg-black/60">
        ...
        <div className="relative space-y-2.5">
          <div className="flex items-center justify-between gap-8 border-b border-white/5 pb-2">
            <span className="uppercase tracking-widest text-white/40">Masterclass Debug v1.0</span>
          ...
          <div className="grid grid-cols-2 gap-x-6 gap-y-1.5">
            <div className="flex flex-col">
              <span className="text-white/30 uppercase text-[8px]">Refresh Rate</span>
              <span className="text-[14px] font-bold text-white leading-none">{fps} <span className="text-[10px] font-normal opacity-50">Hz</span></span>
            </div>
            <div className="flex flex-col">
              <span className="text-white/30 uppercase text-[8px]">Cursor Vector</span>
              <span className="text-white leading-none tracking-tighter">X:{coords.x} Y:{coords.y}</span>
            </div>
            <div className="flex flex-col">
              <span className="text-white/30 uppercase text-[8px]">Resolution</span>
              <span className="text-white leading-none uppercase">{viewport.w}x{viewport.h} px</span>
            </div>
            <div className="flex flex-col">
              <span className="text-white/30 uppercase text-[8px]">GSAP Inst</span>
              <span className="text-white leading-none uppercase">{gsap.globalTimeline.getChildren().length} active</span>
            </div>
          </div>
